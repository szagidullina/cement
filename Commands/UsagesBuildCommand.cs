...
namespace Commands
{
    public class UsagesBuildCommand : CommandBase
    {
        ...
        private void BuildDeps(IEnumerable<Dep> toBuilt)
        {
        ...
        private static void WriteStat(
            IReadOnlyCollection<KeyValuePair<Dep, string>> badParents,
            IReadOnlyCollection<Dep> goodParents)
        {
            if (badParents.Count == 0)
            {
